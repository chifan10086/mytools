...
with open('allzoneid.txt', 'r', encoding='utf-8') as file:
    # 逐行读取文件，并去除末尾的空行
    for line in file:
...
def getns(domain):
    ...
    print(data)
    zoneid = data['Response']['Zones'][0]['OriginalNameServers']
    if len(zoneid) != 0:
        print(domain)
        print(zoneid)
    return zoneid
...
def cha(zoneid):
    ...
    result = pipe.stdout.read().decode()
    data = json.loads(result)
    yz = data['Response']['AccelerationDomains'][0]['OwnershipVerification']
    #print(type(yz))
    if isinstance(yz, dict):
        print(data['Response']['AccelerationDomains'][0]['DomainName'])
   # else:
#        print(zoneid)
